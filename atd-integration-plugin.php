...
function atd_inventory_update_callback( $args, $assoc_args ) {
	...
	if (
		isset( $atd_option['running'] ) &&
		'yes' == $atd_option['running'] &&
		in_array( $tag, array( 'atdw', 'atdt' ) )
	) {
...
XML;
		...
		if ( isset( $matched_product->productSpec ) ) {
			...
			if ( ! isset( $mile_warranty ) || empty( $mile_warranty ) ) {
				...
				$attr_meta['pa_mileage-warranty'] = array(
					'name'         => 'pa_mileage-warranty',
					'value'        => $mile_warranty,
			...
			if ( ! isset( $sidewall ) || empty( $sidewall ) ) {
				...
				$attr_meta                     = get_post_meta( $tire, '_product_attributes', true );
				$attr_meta['pa_sidewall-type'] = array(
					'name'         => 'pa_sidewall-type',
					'value'        => $sidewall,
					'position'     => count( $attr_meta ),
					'is_visible'   => 1,
					'is_variation' => 0,
